...
import random
from other_metric import data_load_function
...
def attack_process(image, pos_input, wo=True):
    if wo is False:
        if ONLY_WHAT == 0:  # blur jpeg gn bright
            processed_image = apply_gaussian_blur(image, pos_input[0], pos_input[1])
        elif ONLY_WHAT == 1:  # jpeg
            processed_image = apply_compression(image, pos_input[2])
        elif ONLY_WHAT == 2:  # gn
            processed_image = apply_gaussian_noise(image, var=pos_input[3])
        elif ONLY_WHAT == 3:  # bright
            processed_image = apply_brightness(image, pos_input[-1], pos_input[4], pos_input[5], pos_input[6])
        else:
            processed_image = apply_gaussian_blur(image, pos_input[0], pos_input[1])
    ...
    if not test_only:
        ...
        for ori_img_path in file_paths:
            ...
            if "SSIM" in mode:
                ...
                param_adv, y_adv, ssim_adv, iter_use = pso_fit(ori_img, adv_img_path)
        ...
        res.append(round(iter_use_sum * 100 / res[0], 3))  # 平均查询次数
...
if __name__ == "__main__":
    ...
    RESULT_PATH = "/AAA200_ALL_RESULT/Ours_Ablation_WO/"
    ...
    METHOD_CHOICE = ["PSO_SSIM", "PSO_FIT"]
    ONLY_CHOICE = ["blur", "jpeg", "gn", "bright"]

    choose_method = 1  # 选择攻击方法  0 PSO_SSIM  1 PSO_FIT
    choose_data = 1  # 选择攻击的数据集  0 ImageNet/ProGAN   1 GenImage/VQDM   2 ImageNet/ldm_200
    ONLY_WHAT = 0  # 0-blur   1-jpeg   2-gn    3-bright
    ONLY_FLAG = ONLY_CHOICE[ONLY_WHAT]
    ...
    if choose_data == 0:  # 非扩散模型
        ...
        test_only = True  # 默认为 False 即先生成再测试
        test_model2attack = [0, 1, 2, 3]  # 想测试的模型
        for i_model in model2attack:
            model_Flag = MODEL_CHOICE[i_model]
            TXT_PATH = RESULT_PATH + f"{save_ATTACK}/{method_ATTACK}/{method_ATTACK}_{model_Flag}_{ONLY_FLAG}.txt"
            SAVE_ROOT_PATH = RESULT_PATH + f"{save_ATTACK}/{method_ATTACK}/{model_Flag}_{ONLY_FLAG}/"
        ...
        model2attack = [5]  # 想攻击的模型  3"FatFormer" 4"UnivFD-Diff" 5"ConvB-Diff"
        test_only = False  # 默认为 False 即先生成再测试
        test_model2attack = [3, 4, 5]  # 想测试的模型
        choose_ckpt = "DRCT"
        for i_model in model2attack:
            model_Flag = MODEL_CHOICE[i_model]
            TXT_PATH = RESULT_PATH + f"{save_ATTACK}/{method_ATTACK}/{method_ATTACK}_{model_Flag}_{ONLY_FLAG}_NEW.txt"
            SAVE_ROOT_PATH = RESULT_PATH + f"{save_ATTACK}/{method_ATTACK}/{model_Flag}_{ONLY_FLAG}_NEW/"
            os.makedirs(SAVE_ROOT_PATH, exist_ok=True)
            with open(TXT_PATH, "a") as f_txt:
                print(dataset_ATTACK, SAVE_ROOT_PATH, file=f_txt)
                print(f"[{gb_Ksize}, {gb_sigma_min}, {gb_sigma_max}, {jpeg_min}, {jpeg_max}, {gn_sigma}, "
